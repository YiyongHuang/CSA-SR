...
def extract_feats():
    ...
    files = glob.glob("dataset/YouTubeClips/*")
    net = tf.keras.applications.InceptionResNetV2(include_top=False, weights='imagenet', pooling='avg')
    ...
    for file in tqdm.tqdm(files):
        ...
        idx = np.linspace(0, len(curr_frames) - 1, 40).astype(int)  # get 40 frames per vid
        ...
        np.save(file.replace('YouTubeClips', 'linear_feats').replace('.avi', '.npy'), curr_feats)
